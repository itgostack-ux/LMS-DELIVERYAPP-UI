...
interface ManifestGroup {
  ...
  // the TRANSIT / ORDER (transferOrderId) + that order's own manifestId.
  manifestId: number;
  manifestNo: string;
...
export class LogiticsUpdate implements OnInit {
  ...
  // ===== Manifest load (Courier + Other Party only) =====
  ...
  // Group by manifestNo (display only) -> bucket each manifest's transits by destination.
  ...
  // Single-row shortcut: advances THIS transit only.
  ...
  // ===== Save: one transaction + one manifest row PER TRANSIT =====
  private updateOrders(
    ...
    for (const order of ordersToAdvance) {
      ...
      // 2) Manifest row for THAT transit only (its own manifestId +
  ...
  // Manifest row for ONE transit. Receiver / OTP come from the per-transit maps
  ...
  sendOtp(): void {
    ...
    // Remember receiver + OTP per transit, then persist one manifest row per
    // transit at its CURRENT status (status only moves on Verify & Deliver).
    ...
    forkJoin(requests).subscribe({
      ...
      <table cellpadding="5">
        ...
        <tr><td><b>Handled By</b></td><td>${this.driverName}</td></tr>
      </table>
      <br>
