...
/**
 * The API returns ONE ROW PER ITEM (IMEI / item code). Several rows share the
 * same transitID. The console works on TRANSITS, so every row below is a TRANSIT
 * and every action fans out to all the item rows behind it.
 */
...
export class LogiticsUpdate implements OnInit {
  ...
  private loadDeliveryLifecycles(loadManifestsAfter: boolean): void {
    ...
    this.logisticsService.getRoleslifecycle(userId).subscribe({
      ...
      next: (roles) => {
        ...
        // Load Complete Lifecycle Master
    ...
    return [...map.entries()]
      .map(([transitID, orders]) => {
        ...
        return {
          ...
          courierName: base.courierName ?? '',
          awbBillNo: base.awbBillNo ?? '',
          otherPartyName: base.otherPartyName ?? '',
  ...
  /** The item row furthest behind — it drives the transit's status. */
...
public nextLifecycleOf(statusCode: string): DeliveryLifecycle | undefined {

  const current = this.findLifecycle(statusCode);
